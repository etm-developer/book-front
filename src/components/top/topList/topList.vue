    ...
    <div class="list_container">
        ...
        <!--尝试加入投票窗口-->
          <div class="award_c_tool"  v-show="this.deToggle">
            <input type="number" v-model="awardNum" placeholder="输入奖赏金额">
            <div class="award_confirm" @click="voteForA(item.id)">确认</div>
          </div>
  ...
  methods: {
    ...
    voteBtn: function () {
      ...
      if (that.$store.state.isLogin === false) {
        that.$store.commit('callToast', {msgHeader: '注意!', msgContent: '仅当您登录后才能使用打赏功能', _confirmfunc: '去登录', _cancelfunc: '关闭', deals: undefined, contract: 3})
        return
      }
    ...
    voteForA: function (id) {
      ...
      this.$store.dispatch('invokeContract', {
        ...
        callback: function (err, msg) {
          ...
          that.$store.commit('callToast', {msgHeader: '成功！', msgContent: '打赏成功！大约十秒后将看到更新信息', _confirmfunc: null, _cancelfunc: null})
          that.$emit('reFresh')
        }
      })
      // 重新初始化
