...
let baseUrl = 'http://129.28.69.70:4096/api/dapps/ca962bcae9adee9c41f880404e5058451d0b67af6a526b06922b85aceb243788'
  ...
  invokeContract: async ({ commit }, {type, args, fee, that, callback}) => {
    ...
    that.$axios.put(posturl, {transaction: a}, { headers: { 'magic': '594fe0f3', 'version': '' } }).then((res) => {
      ...
      if (res.status === 200 && res.data.success) {
        ...
        if (res.data.error.indexOf('Book is not enough') > -1) {
          commit('callToast', {msgHeader: '发生错误', msgContent: '你想购买的书籍数量不足！', _confirmfunc: '了解', _cancelfunc: '关闭', deals: undefined, contract: 4})
        }
